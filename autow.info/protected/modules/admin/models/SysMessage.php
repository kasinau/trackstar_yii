 ...
 */
class SysMessage extends AutoWActiveRecord
{
	/**
...
//			array('create_time, update_time', 'safe'),
    ...
    public static function getLatest()
    {
        ...
        if (($cache = Yii::app()->cache) !== null) {
            $key = 'AutoWorldInfo.SystemMessage';
            if (($sysMessage = $cache->get($key)) !== false) {
                return $sysMessage;
            }
        }
        /**
         * The system message was either not found in the cache, or there is no cache
         * component defined defined for the application,
        ...
        if ($sysMessage != null) {
             ...
             */
            if (isset($key)) {
                $dependency = new CDbCacheDependency('SELECT id FROM sys_message order by update_time DESC');
                $cache->set($key, $sysMessage, 0, $dependency);
            }
            return $sysMessage;
        } else {
            return null;
