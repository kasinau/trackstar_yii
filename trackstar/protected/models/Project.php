...
class Project extends TrackStarActiveRecord
{
    ...
    {
        $time = time();
        $sql = "INSERT INTO tbl_project_user_assignment(project_id, user_id, create_time, create_user_id, update_time, update_user_id)
            VALUES (:project_id, :user_id, FROM_UNIXTIME(:create_time), :create_user_id, FROM_UNIXTIME(:update_time), :update_user_id)";
        $command = Yii::app()->db->createCommand($sql);
        $command->bindValue(":project_id", $this->id, PDO::PARAM_INT);
        $command->bindValue(":user_id", $user->id, PDO::PARAM_INT);
        $command->bindValue(":create_time", $time, PDO::PARAM_INT);
        $command->bindValue(":create_user_id", Yii::app()->user->getId(), PDO::PARAM_INT);
        $command->bindValue(":update_time", $time, PDO::PARAM_INT);
        $command->bindValue(":update_user_id", Yii::app()->user->getId(), PDO::PARAM_INT);
        return $command->execute()==1 ? true : false;
    }
     ...
     * @return bool
     */
    public function deleteUserFromProject($user)
    {
        $sql = "DELETE FROM tbl_project_user_assignment WHERE project_id=:project_id AND user_id=:user_id";
        $command = Yii::app()->db->createCommand($sql);
        $command->bindValue(":project_id", $this->id, PDO::PARAM_INT);
        $command->bindValue(":user_id", $user->id, PDO::PARAM_INT);
        return $command->execute()==1 ? true : false;
